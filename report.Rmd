...
```{r phase-bias, echo = FALSE, fig.cap = "tbc", fig.width = 10}
residuals <- weekly_predictions |>
  left_join(time_means, by = c("week_onset", "variant"),
            suffix = c(".pred", ".data")) |>
  mutate(estimate = estimate.pred - estimate.data,
         lower = estimate.pred - lower.data,
         upper = estimate.pred - upper.data)
growth_by_variant <- weekly_counts |>
  group_by(variant) |>
  mutate(growth = c(diff(n) / head(n, -1), NA_real_)) |>
  ungroup()
res_vs_growth <- residuals |>
  inner_join(growth_by_variant, by = c("week_onset", "variant"))
  
 ggplot(res_vs_growth, aes(x = week_onset, y = estimate,
                               ymin = lower, ymax = upper,
                               colour = variant, fill = variant)) +
  geom_line() +
...
ggplot(res_vs_growth |>
       filter(growth < 2.5),
       aes(x = growth, y = estimate,
           ymin = lower, ymax = upper,
           colour = variant)) +
  geom_point() +
  theme_minimal() +
...
Whilst it has previously been suggested that viral loads may be related to both severity and transmissibility, this does not appear to translate into a relationship that would make it possible to infer these from observed viral loads with there being little clear correlation between trends in viral loads and estimated severity.
