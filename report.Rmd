...
for (idx in seq_along(file_names)) {
  ...
  if (any(grepl("^s_", colnames(draws[[idx]])))) {
    smooths[[idx]] <- conditional_smooths(fits[[idx]])$mu %>%
      mutate(type = type[idx], target_gene = target_gene[idx])
  }
}
...
We modelled individual Ct values from symptomatic pillar 2 values as a proxy for viral load in order to study:

- Ct values by age
- Ct values by variant (possibly interacting with age)
- Ct values by vaccine status (possibly interacting with variant)

We find evidence for heterogeneity by age, and variant with some evidence for variation between vaccine types and doses. Across all variants we found evidence that Ct values decreased with increasing age (corresponding to higher viral loads). The strength of this relationship was decreased for the Delta variant, although the youngest age groups still yield highest Ct values (lowest viral loads).

The Delta variant was associated with lower Ct values (higher viral loads) than either wildtype or the Alpha variant. The size of this effect depends on the target gene: with respect to ORF1ab wildtype and Alpha were associated with very similar Ct values, with the Delta variant being associated with substantially lower Ct values. With respect to N the Alpha variant was associated with lower Ct values than wildtype, with the Delta variant again being associated with lower Ct values yet but less pronouncedly so than with ORF1ab.

# Methods

## Data

Individual Ct values from `r format(nrow(variants), big.mark = ",")` symptomatic pillar 2 cases were extracted for two target genes, ORF1ab and N along with data on the cases age, sex, variant, vaccination status as a combination of vaccine type (AstraZeneca (AZ), Moderna (MD), and Pfizer (PF)) and number of doses (1 or 2), and days between test and symptom onset. Cases were filtered to remove individuals with negative days between test and symptom onset and to remove those with a delay between symptom onset and testing of more than 28 days. Observations with missing data for any of the extracted variables were dropped from further analysis. Combinations of vaccine types, dosages and variants with fewer than 1000 observations were then dropped from the extracted data. These combinations were then combined into a single variable.

## Modelling

We modelled Ct values for each target gene independently and assumed an additive relationship between covariates with a student-t error model. The standard deviation of the student-t error model was allowed to vary by age group using a random effect. All covariates for mean Ct values ($y$) were then modelled as fixed effects with the effect of variants and vaccines being modelled separately for each age group and combined into a group level effect. Interactions between days from symptom onset to test and all other covariates were also included, again as fixed effects. Mathematically this can be represented as follows:
...
The model was implemented using the `brms`  [@brms] package version 2.15.0 in `R` version 4.0.5 [@R] with the `cmdstanr` backend [@cmdstanr]. Default priors from `brms` were used for all parameters. All code required to reproduce this analysis is available from [https://github.com/epiforecasts/covid19_ct_pillar2](https://github.com/epiforecasts/covid19_ct_pillar2). 
...
- We did not use any information on prior infection which could introduce a bias if reinfections occur and affect observed Ct values (e.g., by age, but also by variant as the probability of reinfection probably varies by time and possibly by the variant itself with which a case is infected if there is an element of immune escape).
