...
date: 22 July, 2021. WORK IN PROGRESS.
...
for (idx in seq_along(file_names)) {
  ...
  if (any(grepl("^s_", colnames(draws[[file]][[idx]])))) {
    smooths[[file]][[idx]] <- conditional_smooths(fits[[idx]])$mu %>%
...
with a single group-level factor combining the variant, numer of doses received, vaccine (if vaccinated) and age group. Observations are modelled with a student-t distribution centered around $y$.
